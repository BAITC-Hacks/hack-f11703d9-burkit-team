...
def mock_questions(task: dict[str, Any]) -> dict[str, Any]:
    questions = {
        "ru": [
...
def validate_questions(payload: Any) -> list[dict[str, str]]:
    if not isinstance(payload, dict) or not isinstance(payload.get("questions"), list):
        raise AIResponseError("AI вернул ответ в неизвестном формате.")

    questions = payload["questions"]
    if not 3 <= len(questions) <= 5:
        raise AIResponseError("AI должен вернуть от трёх до пяти вопросов.")

    validated: list[dict[str, str]] = []
    used_fields: set[str] = set()
    for question in questions:
        if not isinstance(question, dict):
            raise AIResponseError("Один из вопросов имеет неверный формат.")
        field = question.get("field")
        text = question.get("text")
        if field not in ALLOWED_QUESTION_FIELDS:
            raise AIResponseError("AI указал неизвестное поле карточки.")
        if field in used_fields:
            raise AIResponseError("AI вернул несколько вопросов для одного поля.")
        if not isinstance(text, str) or not text.strip():
            raise AIResponseError("AI вернул пустой вопрос.")
        used_fields.add(field)
        validated.append({"field": field, "text": text.strip()})
    return validated


def openai_questions(task: dict[str, Any]) -> dict[str, Any]:
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        raise AIUnavailableError(
            "OPENAI_API_KEY не задан. Черновик сохранён; настройте ключ и повторите запрос."
        )
    ...
    try:
        ...
        raise AIUnavailableError(
            "Пакет openai не установлен. Установите зависимости backend."
        ) from error
    ...
    try:
        ...
        raise AIUnavailableError(
            "Не удалось получить ответ от AI. Черновик сохранён; повторите запрос позже."
        ) from error

    if response.status != "completed" or not response.output_text:
        raise AIResponseError("AI не вернул завершённый ответ с вопросами.")

    try:
        payload = json.loads(response.output_text)
    except json.JSONDecodeError as error:
        raise AIResponseError("AI вернул некорректный JSON.") from error
...
def generate_questions(task: dict[str, Any]) -> dict[str, Any]:
    """Выбирает реальный OpenAI или локальный мок по AI_MODE."""
    ...
    if mode == "disabled":
        raise AIUnavailableError(
            "AI-сервис отключён. Черновик сохранён; повторите запрос после включения сервиса."
        )
    raise AIUnavailableError(
        "Неизвестное значение AI_MODE. Допустимые значения: mock, openai, disabled."
    )
