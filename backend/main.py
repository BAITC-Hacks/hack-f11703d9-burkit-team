...
def required_text(payload: dict[str, Any], field: str, label: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise HTTPException(
            status_code=422,
            detail=f"Поле «{label}» обязательно.",
        )
    return value.strip()
...
def create_task(
    ...
    if language not in ALLOWED_LANGUAGES:
        raise HTTPException(
            status_code=422,
            detail="Допустимые языки: kk, ru, en.",
        )
...
def list_tasks(
    ...
    if (
        ...
        and task_status not in TASK_STATUSES
    ):
        raise HTTPException(
            status_code=422,
            detail="Неизвестный статус задачи.",
        )
...
def update_task(
    ...
    if not payload:
        raise HTTPException(
            status_code=422,
            detail="Нет полей для изменения.",
        )
    ...
    if unknown_fields:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Нельзя изменить поля: {fields}.",
        )

    if (
        "language" in payload
        and payload["language"] not in ALLOWED_LANGUAGES
    ):
        raise HTTPException(
            status_code=422,
            detail="Допустимые языки: kk, ru, en.",
    ...
    if "description" in payload:
        payload["description"] = required_text(
            payload,
            "description",
            "Описание",
        )
    ...
    if "author_name" in payload:
        payload["author_name"] = required_text(
            payload,
            "author_name",
            "Автор",
        )
...
def publish_task(task_id: int) -> dict[str, Any]:
    ...
    if task["status"] == "open":
        raise HTTPException(
            status_code=409,
            detail="Задача уже опубликована.",
        )
...
def close_task(task_id: int) -> dict[str, Any]:
    ...
    if task["status"] != "open":
        raise HTTPException(
            status_code=409,
            detail="Закрыть можно только открытую задачу.",
        )
...
def create_proposal(
    ...
    if (
        prototype_url is not None
        and not isinstance(prototype_url, str)
    ):
        raise HTTPException(
            status_code=422,
            detail=(
                "Ссылка на прототип должна "
                "быть строкой."
            ),
        )
